...
export const insertDemo = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const metacloudTargetId = await ctx.db.insert("targetCompanies", {
      userId: user._id,
      companyName: "MetaCloud",
      companyKey: normalizeKey("MetaCloud"),
      website: canonicalizeUrl("https://metacloud.example/careers"),
      domain: "metacloud.example",
      tier: "dream",
      status: "warming_referrals",
      targetRoles: ["Senior Backend Engineer", "Staff Infrastructure Engineer"],
      targetLevel: "Senior / Staff",
      locationPreference: "Remote or NYC",
      workArrangement: "hybrid",
      priorityScore: 95,
      roleFitScore: 82,
      referralGoal: 3,
      researchNotes: "Strong fit for distributed systems and reliability teams.",
      hiringBarNotes: "Expect deep system design, execution stories, and ownership examples.",
      interviewProcessNotes: "Recruiter, coding, system design, behavioral, team match.",
      compensationNotes: "Benchmark against top product-company senior bands.",
      notes: "Do not apply until at least two referral paths are warm.",
      archived: false,
      createdAt: now,
      updatedAt: now,
    })
    const searchlyTargetId = await ctx.db.insert("targetCompanies", {
      userId: user._id,
      companyName: "Searchly",
      companyKey: normalizeKey("Searchly"),
      website: canonicalizeUrl("https://searchly.example/jobs"),
      domain: "searchly.example",
      tier: "strong",
      status: "ready_to_apply",
      targetRoles: ["Product Platform Engineer"],
      targetLevel: "Senior",
      locationPreference: "Remote",
      priorityScore: 84,
      roleFitScore: 88,
      referralGoal: 2,
      researchNotes: "Backend/product intersection is a good resume match.",
      hiringBarNotes: "Prepare product judgment and scalable API examples.",
      interviewProcessNotes: "Likely coding plus system design loop.",
      archived: false,
      createdAt: now,
      updatedAt: now,
    })
    ...
    await Promise.all([
      ctx.db.insert("referralOutreach", {
        userId: user._id,
        targetCompanyId: metacloudTargetId,
        contactName: "Arjun Rao",
        contactRole: "Staff Engineer",
        source: "ex_coworker",
        status: "replied",
        linkedinUrl: canonicalizeUrl("https://www.linkedin.com/in/arjun-rao-demo"),
        firstContactedDate: "2026-05-30",
        lastContactedDate: "2026-06-01",
        followUpDate: "2026-06-08",
        messageTemplate: "Short referral ask with two role links and a tailored resume.",
        notes: "Asked for team-fit advice before requesting referral.",
        archived: false,
        createdAt: now,
        updatedAt: now,
      }),
      ctx.db.insert("referralOutreach", {
        ...
        notes: "Follow up with concise background and target team.",
        archived: false,
        createdAt: now,
        updatedAt: now,
      }),
      ctx.db.insert("referralOutreach", {
        userId: user._id,
        targetCompanyId: searchlyTargetId,
        applicationId: umbrellaId,
        contactName: "Sam Rivera",
        contactRole: "Senior SWE",
        source: "linkedin",
        status: "referred",
        firstContactedDate: "2026-05-25",
        lastContactedDate: "2026-05-28",
        notes: "Referral submitted for infrastructure opening.",
        archived: false,
        createdAt: now,
        updatedAt: now,
      }),
    ...
    await Promise.all([
      ctx.db.insert("interviewPrepPlans", {
        userId: user._id,
        applicationId: acmeId,
        targetCompanyId: metacloudTargetId,
        title: "Acme / MetaCloud system design loop",
        status: "in_progress",
        focusAreas: ["system_design", "behavioral", "resume_deep_dive"],
        ...
        weaknessTags: ["backpressure", "tradeoff articulation"],
        nextAction: "Run a 60-minute mock on event ingestion design.",
        notes: "Tie reliability stories to measurable production impact.",
        createdAt: now,
        updatedAt: now,
      }),
      ctx.db.insert("interviewPrepPlans", {
        userId: user._id,
        targetCompanyId: searchlyTargetId,
        title: "Searchly product-platform prep",
        status: "needs_work",
        focusAreas: ["dsa", "company_research", "behavioral"],
        codingDrillsTarget: 30,
        codingDrillsDone: 6,
        systemDesignDrillsTarget: 3,
        systemDesignDrillsDone: 0,
        behavioralStoriesTarget: 5,
        behavioralStoriesReady: 1,
        mockInterviewsTarget: 2,
        mockInterviewsDone: 0,
        companyResearchDone: false,
        resumeDeepDiveDone: false,
        weaknessTags: ["graphs", "product metrics"],
        nextAction: "Finish graph drills and prepare product tradeoff story.",
        createdAt: now,
        updatedAt: now,
      }),
    ])

    const reliabilityStoryId = await ctx.db.insert("storyBankEntries", {
      userId: user._id,
      title: "Reduced ingestion incidents by redesigning retry flow",
      project: "Event ingestion platform",
      situation: "A high-volume ingestion service was causing recurring incidents during partner traffic spikes.",
      task: "Own the stabilization plan while preserving delivery commitments for the product team.",
      action: "Introduced bounded retries, backpressure, queue visibility, and a staged rollout with rollback gates.",
      result: "Reduced incident frequency and gave on-call engineers actionable dashboards for overload scenarios.",
      impactMetrics: "Cut ingestion-related pages by 45% over the next quarter.",
      technologies: ["Kafka", "TypeScript", "Postgres", "Observability"],
      competencies: ["ownership", "technical_depth", "system_design", "customer_impact"],
      senioritySignal: "Led cross-team tradeoff decisions and set the operational standard for similar services.",
      archived: false,
      createdAt: now,
      updatedAt: now,
    })
    await ctx.db.insert("storyBankEntries", {
      userId: user._id,
      title: "Aligned product and infra teams on platform migration",
      project: "Internal developer platform",
      situation: "Two teams disagreed on whether to pause feature work for a platform migration.",
      task: "Create a migration plan that reduced risk without blocking the roadmap.",
      action: "Split the migration into reversible milestones, clarified ownership, and created weekly risk reviews.",
      result: "The migration shipped without customer-visible downtime and product delivery stayed on track.",
      impactMetrics: "Moved 80% of services in six weeks with no sev-one incidents.",
      technologies: ["Node.js", "React", "CI/CD"],
      competencies: ["collaboration", "ambiguity", "leadership", "execution"],
      senioritySignal: "Drove alignment without formal authority.",
      archived: false,
      createdAt: now,
      updatedAt: now,
    })
    await ctx.db.insert("storyUsages", {
      userId: user._id,
      storyId: reliabilityStoryId,
      applicationId: acmeId,
      interviewId: acmeInterviewId,
      usedAtDate: "2026-06-05",
      confidence: "high",
      notes: "Use for system design tradeoff and ownership prompts.",
      createdAt: now,
      updatedAt: now,
    })
